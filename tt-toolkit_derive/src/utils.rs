...
use syn::{parse::Parse, Attribute, Field};
...
pub fn comma_separated<T: ToTokens>(exprs: impl Iterator<Item = T>) -> TokenStream {
    quote! {
        #(#exprs, )*
    }
}
...
pub trait VariantInfoExt {
    fn construct_from_bindings(&self, f: impl Fn(&BindingInfo) -> TokenStream) -> TokenStream;

    fn find_binding_with_attribute(&self, attr_name: &str) -> Option<BindingInfo>;
}

pub trait StructureExt {
    fn each_construct(&self, f: impl Fn(&BindingInfo) -> TokenStream) -> TokenStream;
}
