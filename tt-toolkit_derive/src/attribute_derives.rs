...
use syn::{Arm, Attribute, Expr, Pat, Token, Type, parse_quote};
...
struct PatternGuard {
    _if: Option<Token![if]>,
    #[parse_if(_if.is_some())]
    expr: Option<Expr>,
}

impl ToTokens for PatternGuard {
...
impl DesugarsToMatchArm for CheckBlock {
    fn desugar(&self, context_type: &Type, entry_type: &Type) -> Arm {
        ...
        parse_quote! {
            (#expr_pat, #type_pat) #guard  => ::core::result::Result::Ok(#body)
        }
    }
}
...
impl DesugarsToMatchArm for BidirSynthBlock {
    fn desugar(&self, context_type: &Type, entry_type: &Type) -> Arm {
        ...
        let optional_attr_type: Type =
            parse_quote!(::core::option::Option<#attr_type>);
        let body = instantiate_dsl(
            context_type,
            &ctx_name(),
            &attr_type,
        ...
        parse_quote! {
            #pattern => ::core::result::Result::Ok({
                ...
                ::core::convert::Into::<#optional_attr_type>::into(__ttt_param)
            })
        }
    }
}
...
fn attr_types(structure: &Structure, attr_name: &str) -> Vec<AttrSpec> {
...
fn opt_check_clause(
    ...
    variant
        .parse_all_attributes::<CheckBlock>("check")
        .into_iter()
...
fn opt_synth_clause(
    ...
    variant
        .parse_all_attributes::<SynthBlock>("synth")
        .into_iter()
        .find(|attr| attr.attr_type == *attr_type)
}

fn opt_bidir_synth_clause(
    variant: &VariantInfo,
    attr_type: &Type,
) -> Option<BidirSynthBlock> {
...
fn derive_check_one(input: &Structure, instance: AttrSpec) -> TokenStream {
    ...
    let check_impl = input.each_variant(|variant| {
        ...
        if let Some(check) = opt_check_clause(variant, attr_type) {
            let bindings = variant.bindings().iter().map(auto_deref);
            let bindings = quote! {
                (
                    ( #(#bindings),* ),
                    #attr_val
            ...
            check.generate_match(
                bindings,
                &instance.context,
                &instance.context_entry,
            )
        } else if let Some(node) = opt_single_binding(variant) {
            quote! {
                #node.check(#ctx_name, __ttt_check_value)
            }
        } else {
            abort! {
...
fn derive_bidir_one(input: &Structure, instance: AttrSpec) -> TokenStream {
    ...
    let check_impl = input.each_variant(|variant| {
        ...
        if let Some(check) = opt_check_clause(variant, attr_type) {
            ...
            quote! {
                #node.check(__ttt_check_value, #ctx_name)
            }
        } else {
            abort! {
                variant.ast().ident.span(),
    ...
    input.gen_impl(quote! {
        gen impl ::ttt::PartialSynthAttribute<#attr_type> for @Self {
            type Ctx = #context_type;
            type Entry = #context_entry;
            type Error = ::ttt::DefaultError;

            fn try_synth(&self, __ttt_context: &#context_type)
...
pub fn derive_attributed(mut input: Structure) -> TokenStream {
    ...
    input.add_bounds(AddBounds::None);

    if !(input.has_attribute(CHECK_TYPES_ATTR)
        || input.has_attribute(SYNTH_TYPES_ATTR)
        || input.has_attribute(BIDIR_TYPES_ATTR))
    {
        abort!(
            Span::call_site(),
